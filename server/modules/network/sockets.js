  ...
  switch (m.shift()) {
    case "k":
      ...
      if (m.length === 1) {
        ...
        if (socket.permissions) {
          ...
          util.log(
            "[WARNING] A socket failed to verify with the token: " + key,
          );
        }
        socket.key = key;
      }
    ...
    case "godmodeButton":
      if (player.body != null && socket.permissions) {
        ...
        player.body.sendMessage(
          player.body.godmode ? "Godmode enabled." : "Godmode disabled.",
        );
      }
      break;
    case "invisibility":
    ...
    case "H":
      ...
      if (body.underControl) {
        if (c.DOMINATOR_LOOP) {
          player.body.sendMessage(
            "You have relinquished control of the dominator.",
          );
          body.giveUp(player, body.isDominator ? "" : undefined);
          return 1;
        } else if (c.MOTHERSHIP_LOOP) {
          player.body.sendMessage(
            "You have relinquished control of the mothership.",
          );
          body.giveUp(player, body.isDominator ? "" : undefined);
          return 1;
        } else {
...
const spawn = (socket, name) => {
  ...
  body.name = name;
  body.sendMessage = (content, displayTime = c.MESSAGE_DISPLAY_TIME) =>
  ...
  player.teamColor = new Color(
    !c.RANDOM_COLORS && (c.GROUPS || (c.MODE == "ffa" && !c.TAG))
      ? 10
      : getTeamColor(body.team),
  ).compiled; // blue
