const { basePolygonDamage, basePolygonHealth } = require("../constants.js"),
  ...
  makeRare = (type, level) => ({
    PARENT: ["food"],
    LABEL:
      ["Shiny", "Legendary", "Shadow", "Rainbow", "Transgender"][level] +
      " " +
      type.LABEL,
    VALUE: [100, 500, 2000, 4000, 5000][level] * type.VALUE,
    SHAPE: type.SHAPE,
    SIZE: type.SIZE + level,
    COLOR: ["lightGreen", "teal", "pureBlack", "rainbow", "animatedTrans"][
      level
    ],
  ...
  makeLaby = (type, level) => {
    ...
    return {
      ...
      SIZE:
        level > 3
          ? Math.max(40, type.SIZE * 2) * (1 + (level - 3) / 6)
          : type.SIZE *
            lerp(2 ** level, 1 + level / 3, Math.min(1, (type.SIZE - 5) / 17)),
      COLOR: type.COLOR,
      ALPHA: type.ALPHA,
      BODY: {
  ...
  TURRETS: [
    {
      ...
      TYPE: [
        "egg",
        { COLOR: { BASE: "white", BRIGHTNESS_SHIFT: -14 }, BORDERLESS: true },
      ],
    },
    {
      ...
      TYPE: [
        "egg",
        { COLOR: { BASE: "white", BRIGHTNESS_SHIFT: -9 }, BORDERLESS: true },
      ],
    },
    {
      ...
      TYPE: [
        "egg",
        { COLOR: { BASE: "white", BRIGHTNESS_SHIFT: -8 }, BORDERLESS: true },
      ],
    },
    {
      ...
      TYPE: [
        "egg",
        { COLOR: { BASE: "white", BRIGHTNESS_SHIFT: -3 }, BORDERLESS: true },
      ],
    },
    {
...
exports.cube = {
  ...
  COLOR: "white",
  SHAPE:
    "M 0.0575 0.0437 V 0.9921 L 0.8869 0.5167 V -0.4306 L 0.0575 0.0437 Z M -0.0583 0.0437 V 0.9921 L -0.8869 0.5159 V -0.4306 L -0.0583 0.0437 Z M 0 -0.0556 L 0.829 -0.5266 L 0 -1 L -0.8254 -0.527 L 0 -0.0556",
  BODY: {
    DAMAGE: 12,
    DENSITY: 20,
    HEALTH: 400,
...
exports.tetrahedron = {
  ...
  COLOR: "white",
  SHAPE:
    "M 0.058 0.044 V 1 L 0.894 -0.434 L 0.058 0.044 Z M -0.0588 0.044 V 1 L -0.894 -0.434 L -0.0588 0.044 Z M 0 -0.056 L 0.8356 -0.5308 L -0.832 -0.5312 L 0 -0.056",
  BODY: {
    DAMAGE: 15,
    DENSITY: 23,
    HEALTH: 500,
...
exports.octahedron = {
  ...
  COLOR: "white",
  SHAPE:
    "M 0.06 -0.06 L 0.95 -0.06 L 0.06 -0.95 L 0.06 -0.06 M -0.06 0.06 L -0.06 0.95 L -0.95 0.06 L -0.06 0.06 M -0.06 -0.06 L -0.95 -0.06 L -0.06 -0.95 L -0.06 -0.06 M 0.06 0.06 L 0.06 0.95 L 0.95 0.06 L 0.06 0.06",
  BODY: {
    DAMAGE: 18,
    DENSITY: 26,
    HEALTH: 600,
...
for (let [gemColor, name] of [
  ...
  exports[name + "BetaPentagonRelic"] = makeRelic(
    exports.betaPentagon,
    -0.6,
    gem,
  );
  exports[name + "AlphaPentagonRelic"] = makeRelic(
    exports.alphaPentagon,
    -0.6,
    gem,
  );
...
exports.tesseract = {
  ...
  COLOR: "white",
  SHAPE:
    "M -0.43 0.35 L -0.71 0.63 L -0.71 -0.63 L -0.43 -0.35 L -0.43 0.35 M -0.35 0.43 L -0.63 0.71 L 0.63 0.71 L 0.35 0.43 L -0.35 0.43 M 0.35 -0.43 L 0.63 -0.71 L -0.63 -0.71 L -0.35 -0.43 L 0.35 -0.43 M 0.43 -0.35 L 0.71 -0.63 L 0.71 0.63 L 0.43 0.35 L 0.43 -0.35 M 0.32 0.32 L 0.32 -0.32 L -0.32 -0.32 L -0.32 0.32 L 0.32 0.32",
  BODY: {
    DAMAGE: 25,
    DENSITY: 40,
    HEALTH: 2000,
...
for (let tier = 0; tier < 6; tier++) {
  for (let poly of ["egg", "square", "triangle", "pentagon", "hexagon"]) {
    for (let shiny of [
      ...
      exports[`laby${tier}${food[0].toUpperCase() + food.slice(1)}`] = makeLaby(
        exports[food],
        tier,
      );
    }
