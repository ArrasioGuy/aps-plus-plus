  ...
  spawn = (tile, team, color, type = false) => {
    ...
    o.on("dead", () => {
      ...
      if (team === TEAM_ENEMIES) {
        ...
        killer = killer
          ? killer.master.master
          : { team: TEAM_ROOM, color: c.MODE === "tdm" ? 3 : 12 };
        ...
        for (let player of sockets.players) {
          if (player.body && player.body.team === newTeam) {
            player.body.sendMessage(
              "Press Insert to take control of the dominator.",
            );
          }
        }
        ...
        if (
          newTeam !== TEAM_ENEMIES &&
          teamcounts[newTeam] >= neededToWin &&
          !gameWon
        ) {
          gameWon = true;
...
let makeDominatorTile = (team, type) =>
    new Tile({ init: (tile) => spawn(tile, team, getTeamColor(team), type) }),
