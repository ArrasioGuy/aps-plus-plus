...
(async function (util, global, settings, Canvas, color, gameDraw, socketStuff) {
    ...
    if (sides instanceof Array) {
      ...
      if ("string" === typeof sides) {
        ...
        if (img.isBroken || !img.complete) {
          // check if img is broken and draw as path2d if so
  ...
  function drawLeaderboard(spacing, alcoveSize, max) {
    ...
    for (let i = 0; i < lb.data.length; i++) {
      ...
      drawBar(
        ...
        y + height / 2,
        height - 3.5,
        global.FFA && entry.id == gui.playerid
          ? color.blue
  ...
  let getDeath = () => {
    ...
    if (global.finalKillers.length) {
      ...
      txt += "❗❗ Get Revenge To Get Your XP Back!";
    }
    return txt;
  };
  const gameDrawDead = () => {
    ...
    drawText("You died!", x, y - 80, 16, color.red, "center");
    drawText(
